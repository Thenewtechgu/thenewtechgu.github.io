  ...
  if (type == "IELTS") {
    ...
    if (stage == 8) {
      if (!progressing) {
        ...
        answers.shift();
        HideAll();
        document.getElementById("results-outer-toeic").className =
          "selbox mainbox";
        //handle AI stuff
        for (let i = 0; i < answers.length; i++) {
          if (typeof questions[i] === typeof [1, 2]) {
            getAIResponseWithImage(
              ...
              document.getElementById("toeic" + (i + 1)).innerHTML =
                parseAIOutput(r);
            });
          } else {
            getAIResponse(
...
alert(
  "I have temporarity disarmed the AI for testing purposes. Someone notify me before Friday. -kien"
);
let id;
